export interface supportDB {
  id: number;
  img: string;
  name: string;
  position: string;
  salary: number;
  office: string;
  skill: string;
  progress: string;
  extn: number;
  email: string;
}

export interface ticketListData {
  statusTitle: string;
  order: number;
  class: string;
  profit: number;
  loss: number;
  level: string;
  levelColor: string;
}

export const supportTicketData: supportDB[] = [
  {
    id: 1,
    img: 'assets/images/user/1.jpg',
    name: 'Tiger Nixon',
    position: 'System Architect',
    salary: 320800,
    office: 'Edinburgh',
    skill: 'primary',
    progress: '30%',
    extn: 5421,
    email: 't.nixon@datatables.net',
  },
  {
    id: 2,
    img: 'assets/images/user/2.png',
    name: 'Garrett Winters',
    position: 'Accountant',
    salary: 170750,
    office: 'Tokyo',
    skill: 'secondary',
    progress: '40%',
    extn: 8422,
    email: 'g.winters@datatables.net',
  },
  {
    id: 3,
    img: 'assets/images/user/3.png',
    name: 'Ashton Cox',
    position: 'Junior Technical Author',
    salary: 486000,
    office: 'San Francisco',
    skill: 'danger',
    progress: '60%',
    extn: 1562,
    email: 'a.cox@datatables.net',
  },
  {
    id: 4,
    img: 'assets/images/user/4.jpg',
    name: 'Cedric Kelly',
    position: 'Senior Javascript Developer',
    salary: 433060,
    office: 'Edinburgh',
    skill: 'secondary',
    progress: '80%',
    extn: 6224,
    email: 'c.kelly@datatables.net',
  },
  {
    id: 5,
    img: 'assets/images/user/5.jpg',
    name: 'Airi Satou',
    position: 'Accountant',
    salary: 162700,
    office: 'Tokyo',
    skill: 'success',
    progress: '70%',
    extn: 5407,
    email: 'a.satou@datatables.net',
  },
  {
    id: 6,
    img: 'assets/images/user/6.jpg',
    name: 'Brielle Williamson',
    position: 'Integration Specialist',
    salary: 372000,
    office: 'New York',
    skill: 'info',
    progress: '55%',
    extn: 4804,
    email: 'b.williamson@datatables.net',
  },
  {
    id: 7,
    img: 'assets/images/user/7.jpg',
    name: 'Herrod Chandler',
    position: 'Sales Assistant',
    salary: 137500,
    office: 'San Francisco',
    skill: 'warning',
    progress: '80%',
    extn: 9608,
    email: 'h.chandler@datatables.net',
  },
  {
    id: 8,
    img: 'assets/images/user/8.jpg',
    name: 'Rhona Davidson',
    position: 'Integration Specialist',
    salary: 327900,
    office: 'Tokyo',
    skill: 'secondary',
    progress: '90%',
    extn: 6200,
    email: 'r.davidson@datatables.net',
  },
  {
    id: 9,
    img: 'assets/images/user/9.jpg',
    name: 'Colleen Hurst',
    position: 'Javascript Developer',
    salary: 205500,
    office: 'San Francisco',
    skill: 'success',
    progress: '24%',
    extn: 2360,
    email: 'c.hurst@datatables.net',
  },
  {
    id: 10,
    img: 'assets/images/user/10.jpg',
    name: 'Sonya Frost',
    position: 'Software Engineer',
    salary: 103600,
    office: 'Edinburgh',
    skill: 'primary',
    progress: '58%',
    extn: 1667,
    email: 's.frost@datatables.net',
  },
  {
    id: 11,
    img: 'assets/images/user/11.png',
    name: 'Jena Gaines',
    position: 'Office Manager',
    salary: 90560,
    office: 'London',
    skill: 'warning',
    progress: '80%',
    extn: 3814,
    email: 'j.gaines@datatables.net',
  },
  {
    id: 12,
    img: 'assets/images/user/12.png',
    name: 'Quinn Flynn',
    position: 'Support Lead',
    salary: 342000,
    office: 'Edinburgh',
    skill: 'danger',
    progress: '70%',
    extn: 9497,
    email: 'q.flynn@datatables.net',
  },
  {
    id: 13,
    img: 'assets/images/user/1.jpg',
    name: 'Charde Marshall',
    position: 'Regional Director',
    salary: 470600,
    office: 'San Francisco',
    skill: 'info',
    progress: '100%',
    extn: 6741,
    email: 'c.marshall@datatables.net',
  },
  {
    id: 14,
    img: 'assets/images/user/2.png',
    name: 'Haley Kennedy',
    position: 'Senior Marketing Designer',
    salary: 313500,
    office: 'London',
    skill: 'success',
    progress: '50%',
    extn: 3597,
    email: 'h.kennedy@datatables.net',
  },
  {
    id: 15,
    img: 'assets/images/user/3.png',
    name: 'Tatyana Fitzpatrick',
    position: 'Regional Director',
    salary: 385750,
    office: 'London',
    skill: 'warning',
    progress: '30%',
    extn: 1965,
    email: 't.fitzpatrick@datatables.net',
  },
  {
    id: 16,
    img: 'assets/images/user/4.jpg',
    name: 'Michael Silva',
    position: 'Marketing Designer',
    salary: 198500,
    office: 'London',
    skill: 'primary',
    progress: '20%',
    extn: 1581,
    email: 'm.silva@datatables.net',
  },
  {
    id: 17,
    img: 'assets/images/user/5.jpg',
    name: 'Paul Byrd',
    position: 'Chief Financial Officer (CFO)',
    salary: 725000,
    office: 'New York',
    skill: 'success',
    progress: '10%',
    extn: 3059,
    email: 'p.byrd@datatables.net',
  },
  {
    id: 18,
    img: 'assets/images/user/6.jpg',
    name: 'Gloria Little',
    position: 'Systems Administrator',
    salary: 237500,
    office: 'New York',
    skill: 'success',
    progress: '15%',
    extn: 1721,
    email: 'g.little@datatables.net',
  },
  {
    id: 19,
    img: 'assets/images/user/7.jpg',
    name: 'Bradley Greer',
    position: 'Software Engineer',
    salary: 132000,
    office: 'London',
    skill: 'primary',
    progress: '30%',
    extn: 2558,
    email: 'b.greer@datatables.net',
  },
  {
    id: 20,
    img: 'assets/images/user/8.jpg',
    name: 'Dai Rios',
    position: 'Personnel Lead',
    salary: 217500,
    office: 'Edinburgh',
    skill: 'warning',
    progress: '40%',
    extn: 2290,
    email: 'd.rios@datatables.net',
  },
  {
    id: 21,
    img: 'assets/images/user/9.jpg',
    name: 'Jenette Caldwell',
    position: 'Development Lead',
    salary: 345000,
    office: 'New York',
    skill: 'primary',
    progress: '20%',
    extn: 1937,
    email: 'j.caldwell@datatables.net',
  },
  {
    id: 22,
    img: 'assets/images/user/10.jpg',
    name: 'Yuri Berry',
    position: 'Chief Marketing Officer (CMO)',
    salary: 675000,
    office: 'New York',
    skill: 'danger',
    progress: '60%',
    extn: 6154,
    email: 'y.berry@datatables.net',
  },
  {
    id: 23,
    img: 'assets/images/user/11.png',
    name: 'Caesar Vance',
    position: 'Pre-Sales Support',
    salary: 106450,
    office: 'New York',
    skill: 'success',
    progress: '50%',
    extn: 8330,
    email: 'c.vance@datatables.net',
  },
];

export const ticketListStatus: ticketListData[] = [
  {
    statusTitle: 'Order',
    order: 2563,
    profit: 8989,
    loss: 2560,
    level: '70%',
    class: 'bg-light-primary txt-primary',
    levelColor: 'progress-striped-primary',
  },
  {
    statusTitle: 'Pending',
    order: 8943,
    profit: 8989,
    loss: 2560,
    level: '70%',
    class: 'bg-light-secondary txt-secondary',
    levelColor: 'progress-striped-secondary',
  },
  {
    statusTitle: 'Running',
    order: 2500,
    profit: 8989,
    loss: 2560,
    level: '70%',
    class: 'bg-light-warning txt-warning',
    levelColor: 'progress-striped-warning',
  },
  {
    statusTitle: 'Smooth',
    order: 2060,
    profit: 8989,
    loss: 2560,
    level: '70%',
    class: 'bg-light-info txt-info',
    levelColor: 'progress-striped-info',
  },
  {
    statusTitle: 'Done',
    order: 5600,
    profit: 8989,
    loss: 2560,
    class: 'bg-light-success txt-success',
    level: '70%',
    levelColor: 'progress-striped-success',
  },
  {
    statusTitle: 'Cancel',
    order: 2560,
    profit: 8989,
    loss: 2560,
    level: '70%',
    class: 'bg-light-danger txt-danger',
    levelColor: 'progress-striped-danger',
  },
];
